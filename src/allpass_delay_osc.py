...
gen = ["Series/fmnet", [["Fanout/fo",["ADSR/adsr","APDelayOsc/saw"]],"Product/mul", "SoundFileSink/dest2"]]

# Create network and intialize parameter mapping 
network = create(gen)

network.updControl("Fanout/fo/APDelayOsc/saw/mrs_real/frequency", 440.0)
network.updControl("Fanout/fo/APDelayOsc/saw/mrs_natural/type", 1)
...
# Set up Audio File
network.updControl( "SoundFileSink/dest2/mrs_string/filename", "APDelayTest.wav")
...
for note in notes:
    time = 0.0
    nton = 'on'
    network.updControl("Fanout/fo/APDelayOsc/saw/mrs_real/frequency", note)
    network.updControl("Fanout/fo/APDelayOsc/saw/mrs_bool/noteon", MarControlPtr.from_bool(True))
